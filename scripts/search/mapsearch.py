...
def map_iteration(active_pm, check_pm, map_pms, check_map, current_plan, iteration, additions, prev_state = [], exp_actions=[]):
        ...
        for chain in chains:
            for achain in active_chains:
                if chain[-1].sign == achain[-1].sign and len(chain) > 2 and chain not in merged_chains:
                    ...
                    break
        scripts = _generate_meanings(merged_chains, agents, I_obj)
    ...
    for counter, name, script, ag_mask in candidates:
        ...
        if logic == 'spatial':
            ...
            next_pm = _time_shift_forward(active_pm, script)
...
def _generate_meanings(chains, agents, I_obj):
    ...
    if constraints:
        ...
        for ag, actions in mapped_actions.items():
            ...
            if ag.name == 'I':
                agcall = I_obj.name
            if not agcall in constraints:
                continue
            else:
                constrain = constraints[agcall]
            variants = []
            for constr_role, predicates in constrain.items():
                merged = merge_predicates(predicates)
                for sets in merged:
                    consist = [set(), []]
                    for pred in sets:
...
def _check_activity(pm, active_pm):
    ...
    for event in pm.cause:
        ...
        for fevent in active_pm.cause:
            ...
            if event.resonate('meaning', fevent, True, logic):
                break
        else:
            result = False
            break
...
def _meta_check_activity(scripts, active_pm, check_pm, prev_pms, additions, iteration, prev_state, check_map):
    ...
    if logic == 'spatial':
        ...
        for agent, script in scripts:
            estimation = _time_shift_forward(active_pm, script)
            for prev in prev_pms:
                if estimation.resonate('meaning', prev, False, False):
                    break
            else:
