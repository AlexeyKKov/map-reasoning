...
def spatial_ground(problem, agent, logic):
    ...
    # ground types
    for type_name, smaller in problem.domain['types'].items():
    ...
    for role_name, smaller in problem.domain['roles'].items():
    ...
    # ground predicates
    for predicate_name, smaller in problem.domain['predicates'].items():
    ...
    events = []
    # ground actions
    for action_name, smaller in problem.domain['actions'].items():
        matrices = []
    ...
    region_map, cell_map, cell_location, near_loc, cell_coords = signs_markup(initial_state, agent)
    ...
    region_map, cell_map, cell_location, near_loc, _ = signs_markup(goal_state, agent)
    ...
    return Task(problem.name, signs, None, start_situation.sign, goal_situation.sign, logic, goal_map.sign, map_pms, additions)
